...
| Gate 失败止损计数器 | v4.0.30 | ✅ **默认生效（本地台账）** | 同一 change/phase/check 连续失败第 3 次起输出强止损提示，要求暂停盲目修补并转人工决策。边界：这是 circuit breaker 提示，不是硬禁止继续执行 |
...
优先事项：
- **gate 失败计数器**：同一个 check 连续失败 N 次（如 3 次）→ 强制暂停，提示"向用户汇报，而不是继续尝试修复"，给出"跳过 / 指导修复 / 终止"三选项。✅ v4.0.30 已完成本地台账 + 止损提示。
...
- **v4.0.30 — 止损计数器**：gate 同一 check 连续失败 N 次强制暂停转人工。
  - 验收：连续 fix 死循环在第 N 次被掐断。✅ 已完成（提示型 circuit breaker）
