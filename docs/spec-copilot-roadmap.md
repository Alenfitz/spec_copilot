...
| Guard hash 校验 / Contract Freeze | v2.6 / v4.0.18 | ⚠️ **需手动启用，否则形同虚设** | `install` 不自动装 guard；未安装时 gate 的 hash 校验走 `catch{}` **静默放行**。test05 中 AI 能随意篡改/合并功能点，根因正是此机制从未上膛 |
...
- **v4.0.27 — Guard 默认上膛**：`install` 默认 `guard install`；gate 的 `catch{}` 改成区分"未安装(强警告)/校验失败(拦截)"；doctor 把"guard 未安装"从 info 升级为 issue。
  - 验收：重跑 test05 路径，AI 改 spec.md 后 gate 必然拦截。
- **v4.0.28 — 哨兵防伪 + 止损计数器**：确认并修复 `.gate-*-passed` 可伪造问题；gate 同一 check 连续失败 N 次强制暂停转人工。
