...
- `/spec:init` 是每次会话的**唯一合法起点**。
- 如果用户在同一条消息里同时说了"完成开发"、"先写代码"、"直接实现"，也**不能**跳过本命令。
- 本命令完成前，禁止进入 `/spec:bootstrap` 之外的任何开发阶段。
...
> 如果本次判定为空壳项目，则结束语必须改为：
> `规范已加载 ✓`
> `→ 下一步：/spec:bootstrap`
> 不得输出 propose/apply/direct code 之类的后续建议。
