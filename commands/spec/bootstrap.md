...
- `/spec:bootstrap` 完成前，禁止进入 `/spec:propose`、`/spec:lite`、`/spec:apply` 或直接写业务代码。
- 用户如果说"顺手把功能也做了"、"脚手架后直接开发"，也必须先完成本命令并停在"可以开始第一个需求"。
- 本命令允许生成**可编译/可启动的空骨架**，但禁止加入任何业务功能实现。
...
> 输出到这里必须停止。即使用户此前说过"初始化完直接开发"，也不能越过 propose/lite 阶段。
