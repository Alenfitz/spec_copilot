...
`gate review` 的加权功能点覆盖率用 `/F\d{2,}/g` 从整篇 spec 抓功能点 ID。如果 spec 散文里写了
"业需 F01-F23 中无独立功能点"这类**范围引用**,正则会把端点 `F23` 当成一个独立声明的功能点;
而它并无实现 → 覆盖率分母虚高、出现永远命中不了的"幽灵功能点"(实测覆盖率被压到 50%)。

修复:新增 `extractFeaturePointIds`,提取前先剥掉 `F01-F23 / F01~F23 / F01 至 F23 / F01 到 F23`
等范围引用(支持 `- ~ ～ – — 至 到` 连接符);范围两端若另有独立声明则仍保留。

效果:spec 里出现功能点范围描述不再污染覆盖率。新增 4 例回归测试;全套 132 测试通过。
...
本版本不声称已经解决“复杂需求一定会被拆得完美”这个问题。它先解决一个更窄但真实的问题：当 `tasks.md` 只是按“前端一批 / 后端一批 / 测试一批”或“大 task 一把梭”拆分时，AI 更容易铺开很多文件、每层只做一半，最后靠自评解释缺口。

#### `tasks.md` 模板新增 V-Slice 闭环字段

- 每个 task 新增 `V-Slice（必填）`
- 必填字段包括：
  - `用户动作`
  - `接口契约`
  - `状态/数据变化`
...
效果：task 不再只描述“改哪些文件”，而必须说明“用户最终能做成什么，以及如何验证”。

#### `/spec:propose` 新增 task 拆分规则

- 明确禁止只按技术层拆 task
- 明确要求每个 task 生成 `V-Slice`
- 明确要求每个 task 至少写出一条 `不可降级项`
- 给出推荐拆分示例：
  - 列表查询闭环
  - 新建闭环
  - 保存闭环

效果：把“合理拆 task”的要求前移到 propose 阶段，而不是等 apply/review 再被动暴露。

#### apply gate 新增 Task Vertical Slice 检测

- `gate apply` 会检查每个 task 是否包含完整 V-Slice 字段
- 缺少闭环字段时，apply gate 失败
- 单 task 覆盖功能点超过 `3` 个时，apply gate 失败
- `不可降级项` 写成 `无` 时给出 warning，提醒补充硬验收点

效果：把“task 切得过大 / 过空 / 只有技术动作没有业务闭环”的问题拦在编码前。

#### lint 前移结构检查

- `spec-lint.sh` 现在会检查：
  - 每个 task 是否有 `V-Slice（必填）` 区块
  - 每个 task 是否有 `不可降级项`
  - 每个 task 是否缺少 V-Slice 必填字段
  - 单 task 覆盖功能点是否超过 `3` 个

效果：让 propose 阶段就能尽早暴露任务结构问题，而不是等到真正写代码后才发现 task 设计不合理。
...
- 缺少 V-Slice 字段时失败
- 单 task 覆盖超过 3 个功能点时失败
- lint 能拦截缺少 V-Slice / 不可降级项的 task
- 合法 V-Slice task 能通过 lint
...
- 这版还不能自动判断 task 的业务边界是否“最优”
- 这版先做结构层面的约束，不做复杂语义理解
- 旧模板或历史 change 可能仍需逐步迁移

后续如果继续补这一条线，重点应放在：

- 契约闭环检查
- 更合理的 task 类型识别
- 把 “不可降级项” 和后续 review/test 进一步联动
